   ...
   , m_endcapPixelLayer(                iConfig.getParameter<int>("endcapPixelLayer"))
{
  ...
  for (std::vector<std::string>::const_iterator muonSelector = m_muonSelectors_strings.begin();  muonSelector != 
m_muonSelectors_strings.end();  ++muonSelector) {
    m_muonSelectors.push_back(muon::selectionTypeFromString(*muonSelector));
  }

  for (std::vector<edm::ParameterSet>::const_iterator detailedSelector = m_detailedSelectors.begin();  detailedSelector != 
m_detailedSelectors.end();  ++detailedSelector) {
    ...
    else if (arbitrationType == std::string("SegmentArbitration")        ) 
m_detailed_arbitrationType.push_back(reco::Muon::SegmentArbitration);
    else if (arbitrationType == std::string("SegmentAndTrackArbitration")) 
m_detailed_arbitrationType.push_back(reco::Muon::SegmentAndTrackArbitration);
    else {
      throw cms::Exception("BadConfig") << "arbitrationType must be one of \"NoArbitration\", \"SegmentArbitration\", 
\"SegmentAndTrackArbitration\"" << std::endl;
    }
  }
}
...
bool MuJetProducerRun3::muonOkay(const pat::Muon &muon) {
  ...
  //else if ( muon.outerTrack().isAvailable() && ( muon.outerTrack()->pt() < m_minPt || fabs(muon.outerTrack()->eta()) > m_maxAbsEta ) ) 
return false;
  ...
  for (std::vector<muon::SelectionType>::const_iterator muonSelector = m_muonSelectors.begin();  muonSelector != m_muonSelectors.end();  
++muonSelector) {
  ...
  for (;  algorithmType != m_detailed_algorithmType.end();  ++algorithmType, ++minNumberOfMatches, ++maxAbsDx, ++maxAbsPullX, ++maxAbsDy, 
++maxAbsPullY, ++maxChamberDist, ++maxChamberDistPull, ++arbitrationType) {
    if (!muon::isGoodMuon(muon, *algorithmType, *minNumberOfMatches, *maxAbsDx, *maxAbsPullX, *maxAbsDy, *maxAbsPullY, *maxChamberDist, 
*maxChamberDistPull, *arbitrationType)) return false;
  }
...
void MuJetProducerRun3::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
 ...
 //edm::Handle<reco::TransientTrackBuilder>  transientTrackBuilder;
  ...
  //std::cout << "*******************************" << std::endl;
  //std::cout << "BeamSpot:    (x,y,z)[cm]: "<< beamSpot->position().x() << ", " << beamSpot->position().y() <<", "<< beamSpot->position().z() 
<<std::endl;
  double MuonCount=0;
  for (pat::MuonCollection::const_iterator mui = muons->begin();  mui != muons->end();  ++mui) {
    ...
    //std::cout << "                 pT[GeV]: " << mui->pt() << "; eta: " << mui->eta() << "; phi: " << mui->phi() << "; q: " << mui->charge() 
<< "; Tracker Mu: " << mui->isTrackerMuon() << "; Global Mu: " << mui->isGlobalMuon() << "; PF Mu:" << mui->isPFMuon() <<std::endl;
    ...
    */
    ...
    if(mui->genParticle() != 0){
      //std::cout << "Matched GEN: (x,y,z)[cm]: "<< mui->genParticle()->vx() << ", " <<mui->genParticle()->vy() <<", "<< 
mui->genParticle()->vz() <<std::endl;
      //std::cout << "                 pT[GeV]: "<< mui->genParticle()->pt() << "; eta: " << mui->genParticle()->eta() <<"; phi: "<< 
mui->genParticle()->phi() <<std::endl;
...
/*
  for (reco::TrackCollection::const_iterator muj = DSAmuons->begin();  muj != DSAmuons->end();  ++muj) {
    ...
    std::cout << "DSA Mu     (x,y,z)[cm]: "<< muj->vx() - beamSpot->position().x() <<", "<< muj->vy() - beamSpot->position().y() <<", "<< 
muj->vz() - beamSpot->position().z() <<std::endl;
    std::cout << "               pT[GeV]: "<< muj->pt() <<"; eta: "<< muj->eta() <<"; phi: "<< muj->phi() << "; Q: " << muj->charge() 
<<std::endl;
  ...
  for (reco::TrackCollection::const_iterator ONE = DSAmuons->begin();  ONE != DSAmuons->end();  ++ONE) {
    for (reco::TrackCollection::const_iterator TWO = ONE;  TWO != DSAmuons->end();  ++TWO) {
      ...
      && (ONE->charge() != TWO->charge()) && (P4ONE + P4TWO).M() < 60.0 ){
        ...
        std::cout << "    mu ONE   (x,y,z)[cm]: " << ONE->vx() - beamSpot->position().x() <<", "<< ONE->vy() - beamSpot->position().y() <<", 
"<< ONE->vz() - beamSpot->position().z() <<std::endl;
        std::cout << "                 pT[GeV]: " << ONE->pt() <<"; eta: "<< ONE->eta() <<"; phi: "<< ONE->phi() << "; Q: " << ONE->charge() 
<<std::endl;
        std::cout << "       TWO   (x,y,z)[cm]: " << TWO->vx() - beamSpot->position().x() <<", "<< TWO->vy() - beamSpot->position().y() <<", 
"<< TWO->vz() - beamSpot->position().z() <<std::endl;
        std::cout << "                 pT[GeV]: " << TWO->pt() <<"; eta: "<< TWO->eta() <<"; phi: "<< TWO->phi() << "; Q: " << TWO->charge() 
<<std::endl;
        ...
        if( DimuVtx.isValid() ){
          ...
          std::cout << "         vtx (x,y,z)[cm]: " << DimuVtx.position().x() << ", " << DimuVtx.position().y() << ", " << 
DimuVtx.position().z()<<std::endl;
        }
        else{
  ...
  for (pat::MuonCollection::const_iterator one = muons->begin();  one != muons->end();  ++one) {
    if (muonOkay(*one)) {
	    for (pat::MuonCollection::const_iterator two = one;  two != muons->end();  ++two) {
          ...
          else if (m_groupingMode == kGroupByDeltaROrMassAndVertexProb) satisfied = (satisfied_deltaR ||  satisfied_mass)        &&  
satisfied_vertexProb;
          else if (m_groupingMode == kGroupByMassAndVertexProbOrDeltaR) satisfied = satisfied_mass    &&  (satisfied_vertexProb  ||  
satisfied_deltaR);
          else     assert(false);
          ...
          std::cout << "             pT[GeV]: " << one->pt() << "; eta: " << one->eta() << "; phi: " << one->phi() << "; q: " << one->charge() 
<< "; Tracker Mu: " << one->isTrackerMuon() << "; Global Mu: " << one->isGlobalMuon() << "; PF Mu:" << one->isPFMuon() << "; SA Mu: " << 
one->isStandAloneMuon() <<std::endl;
          if( one->innerTrack().isAvailable() )    std::cout << "  innerTrack pT[GeV]: " << one->innerTrack()->pt()    << "; eta: " << 
one->innerTrack()->eta()    << "; phi: " << one->innerTrack()->phi()    <<std::endl;
          if( one->outerTrack().isAvailable() )    std::cout << "  outerTrack pT[GeV]: " << one->outerTrack()->pt()    << "; eta: " << 
one->outerTrack()->eta()    << "; phi: " << one->outerTrack()->phi()    <<std::endl;
          if( one->globalTrack().isAvailable() )   std::cout << " globalTrack pT[GeV]: " << one->globalTrack()->pt()   << "; eta: " << 
one->globalTrack()->eta()   << "; phi: " << one->globalTrack()->phi()   <<std::endl;
          if( one->muonBestTrack().isAvailable() ) std::cout << "   bestTrack pT[GeV]: " << one->muonBestTrack()->pt() << "; eta: " << 
one->muonBestTrack()->eta() << "; phi: " << one->muonBestTrack()->phi() <<std::endl;

          std::cout << "Muon two (x,y,z)[cm]: " << two->vx() <<", "<< two->vy() <<", "<< two->vz() <<std::endl;
          std::cout << "             pT[GeV]: " << two->pt() << "; eta: " << two->eta() << "; phi: " << two->phi() << "; q: " << two->charge() 
<< "; Tracker Mu: " << two->isTrackerMuon() << "; Global Mu: " << two->isGlobalMuon() << "; PF Mu:" << two->isPFMuon() << "; SA Mu: " << 
two->isStandAloneMuon() <<std::endl;
          if( two->innerTrack().isAvailable() )    std::cout << "  innerTrack pT[GeV]: " << two->innerTrack()->pt()    << "; eta: " << 
two->innerTrack()->eta()    << "; phi: " << two->innerTrack()->phi()    <<std::endl;
          if( two->outerTrack().isAvailable() )    std::cout << "  outerTrack pT[GeV]: " << two->outerTrack()->pt()    << "; eta: " << 
two->outerTrack()->eta()    << "; phi: " << two->outerTrack()->phi()    <<std::endl;
          if( two->globalTrack().isAvailable() )   std::cout << " globalTrack pT[GeV]: " << two->globalTrack()->pt()   << "; eta: " << 
two->globalTrack()->eta()   << "; phi: " << two->globalTrack()->phi()   <<std::endl;
          if( two->muonBestTrack().isAvailable() ) std::cout << "   bestTrack pT[GeV]: " << two->muonBestTrack()->pt() << "; eta: " << 
two->muonBestTrack()->eta() << "; phi: " << two->muonBestTrack()->phi() <<std::endl;

          std::cout << "muonPair fitted vtx valid: " << muonPair.vertexValid() <<", dR: "<< muonPair.dR(0, 1, muonPair.vertexValid()) 
<<std::endl;
          if( muonPair.vertexValid() ) std::cout << "muonPair fitted vtx mass: " << muonPair.vertexMass() << ", prob.: "<< 
muonPair.vertexProb() <<std::endl;*/
          //End DEBUG@WeiShi
  ...
  for (pat::MuonCollection::const_iterator muon = muons->begin();  muon != muons->end();  ++muon) {
    if (muonOkay(*muon)) {
      ...
      //std::cout << "         pT[GeV]: " << muon->pt() << "; eta: " << muon->eta() << "; phi: " << muon->phi() << "; q: " << muon->charge() 
<< "; Tracker Mu: " << muon->isTrackerMuon() << "; Global Mu: " << muon->isGlobalMuon() << "; PF Mu:" << muon->isPFMuon() <<std::endl;
  ...
  if (FinalJets.size() != 0){
  std::cout<<"line "<< __LINE__<<"  FinalJets muo 1 ptr  "<<  FinalJets.at(0).muon(0)<<"  FinalJets muon 1 source ptr  "<< 
FinalJets.at(0).muon(0)->sourceCandidatePtr(1).get()<<std::endl;
